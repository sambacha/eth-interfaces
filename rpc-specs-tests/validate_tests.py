...
def run_test(test_name, session, rpc_url, reporter):
    ...
    for test_case in test_spec["tests"]:
        ...
        if (
            requestSchemaIsValid is True
            and test_case["request"]["shouldFailSchema"] is False
        ) or (
            requestSchemaIsValid is False
            and test_case["request"]["shouldFailSchema"] is True
        ):
            print("rpc request schema validated.")
            reporter.subresult(
                test_case["title"] + ". rpc request schema valid", True, None, None
            )
        else:
            ...
            reporter.subresult(
                test_case["title"] + ". rpc request schema valid",
                False,
                err_msg,
                requestSchemaError,
            )
        ...
        try:
            ...
            reporter.subresult(
                test_case["title"] + ". rpc expected response schema valid",
                True,
                None,
                None,
            )
        ...
        assert_dict = {
            "receivedResponse": expected_response,
        ...
        for assertion in test_case["asserts"]:
            ...
            if assertion_result == True:
                reporter.subresult(
                    test_case["title"] + ". " + assertion["description"] + ".",
                    True,
                    None,
                    None,
                )
                ...
                reporter.subresult(
                    test_case["title"] + ". " + assertion["description"] + ".",
                    False,
                    "assertion failed: " + assertion["program"],
                    assert_dict,
                )
